...
export default function NewBorrowRequestPage() {
    ...
    const handle = window.setTimeout(async () => {
      try {
        const resp = await api<InventoryItem[]>(
          `/api/inventory?itemType=NON_CONSUMABLE&search=${encodeURIComponent(term)}`,
          { method: "GET" }
        );
        setItems(resp);
      } catch (e: any) {
  ...
  if (!me.roles?.includes("Driver")) {
    return (
      <div>
        ...
        <PageHeader title="New Borrow Request" />
  ...
  const submit = async () => {
    ...
    try {
      ...
      show("Borrow request submitted.", "success");
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        description="Non-consumables only. Asset required."
        actions={
      ...
      <div className="mt-6">
        ...
        {lines.length === 0 ? (
          <EmptyState title="No lines yet." description="Add at least one non-consumable line." />
        ) : null}
      </div>
